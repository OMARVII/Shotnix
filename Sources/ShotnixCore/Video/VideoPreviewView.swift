        ...
        DragGesture(minimumDistance: 0, coordinateSpace: .named(Self.space))
            .onChanged { value in
                ...
                model.updateOverlay(origin.id, coalesce: "move-\(origin.id)") { overlay in
                    overlay.x = origin.x + Double(value.translation.width / max(stage.width * viewScale, 1))
                    overlay.y = origin.y + Double(value.translation.height / max(stage.height * viewScale, 1))
                }
            }
            .onEnded { value in
