...
extension String {
	...
	Patched for Shotnix (vendored copy): the SwiftPM-generated `Bundle.module`
	accessor only searches the .app root and the build machine's absolute
	`.build` path, so in a released app it fatalErrors on every machine except
	the one that built it (Shotnix issue #25). Locate the resource bundle
	where the app actually ships it instead, and fall back to the unlocalized
...
extension Bundle {
	/// Resilient replacement for the generated `Bundle.module` accessor.
	fileprivate static let keyboardShortcutsResources: Bundle? = {
		...
		let candidates: [URL?] = [
			...
			Bundle.main.executableURL?.deletingLastPathComponent()  // swift build dev layout
		]
