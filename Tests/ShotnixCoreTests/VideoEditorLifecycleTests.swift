...
final class VideoEditorLifecycleTests: XCTestCase {
    ...
    func testExportIsAnnouncedToTheQuitPromptUntilItEnds() async throws {
        ...
        // A quit waits for the file instead of cutting it off.
        var quitGoesAhead = false
        AppTermination.finishAll { quitGoesAhead = true }
        for _ in 0..<200 where model.isExporting {
            await T.settle(0.05)
        }
        XCTAssertFalse(model.isExporting)
        if case .finished = model.exportPhase {} else { XCTFail("finished: \(model.exportPhase)") }
